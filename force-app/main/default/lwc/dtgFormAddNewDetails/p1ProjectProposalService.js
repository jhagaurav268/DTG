...
export default class P1ProjectProposalService {
        ...
        if (event.target.name === 'Token') {
            ...
            if (tokendata) {
                ...
                fetchRecordDetails({ filter: filter, context: this.parent.context, origin: window.location.origin + '/GrantmakingPortal' }).then(res => {
                    ...
                    this.parent.data[0].Demo_Rich_Text__c = res.attachments.Demo_Rich_Text__c
                        ? `<p><img src="${res.attachments.Demo_Rich_Text__c.attachmentUrl}"/></p><p>${res.attachments.Demo_Rich_Text__c.richText || ''}</p>`
                        : '';
                    this.parent.imageData = res.attachments.Demo_Rich_Text__c
                        ? `<p><img src="${res.attachments.Demo_Rich_Text__c.attachmentUrl}"/></p><p>${res.attachments.Demo_Rich_Text__c.richText || ''}</p>`
                        : '';

                    this.parent.data[0].Demo_Rich_Text_2__c = res.attachments.Demo_Rich_Text_2__c
                        ? `<p><img src="${res.attachments.Demo_Rich_Text_2__c.attachmentUrl}"/></p><p>${res.attachments.Demo_Rich_Text_2__c.richText || ''}</p>`
                        : '';
                    this.parent.imageDataTwo = res.attachments.Demo_Rich_Text_2__c
                        ? `<p><img src="${res.attachments.Demo_Rich_Text_2__c.attachmentUrl}"/></p><p>${res.attachments.Demo_Rich_Text_2__c.richText || ''}</p>`
                        : '';
                    this.parent.currentScreen = this.__loadDynamicInputs(JSON.parse(JSON.stringify(this.parent.currentScreen)));
                }).catch(ex => {
    ...
    stepClick(e) {
        if (this.parent.currentStep === 6 && e.target.value < this.parent.currentStep) {
            __errorMessage('Cannot Navigate Back', 'You cannot go back to previous step', this.parent);
            e.target.classList.remove('slds-is-active');
            e.preventDefault();
            return;
        }
        // if (this.parent.currentStep <= e.target.value) {
        //     if (!__validateInputs(this.parent.template.querySelectorAll(".table-container>table>tbody>tr>td"))) {
        //         __errorMessage(' Required Fields Missing!', ' Please Fill The Required Fields!', this.parent);
        //         return;
        //     }
        // }


        if (e.target.value >= this.parent.screens.length - 1) {
            __errorMessage('Submit the Application First', 'You cannot navigate to submit tab without submitting the report', this.parent);
            return;
        }
        this.parent.data = __processInputData(this.parent.data, this.parent.template.querySelectorAll(".table-container>table>tbody>tr>td"));
    ...
    saveAsDraft() {
        ...
        this.parent.loadingSpinner = true;
        upsertData({ data: datum, recordId: this.parent.recordId, context: this.parent.context, isSubmit: false }).then(res => {
            console.log('res=>', res);
    ...
    submit() {
        ...
        datum = __stringifyData(datum);
        this.parent.loadingSpinner = true;
        upsertData({ data: datum, recordId: this.parent.recordId, context: this.parent.context, isSubmit: true }).then(res => {
            console.log('res=>', res);
            this.parent.recordId = res.id;
            this.parent.loadingSpinner = false;
            __successMessage(this.parent.context + ' Submitted!', 'A recod of ' + this.parent.context + ' submitted Successfully!', this.parent);
            if (res.id) {
                this.next();

                setTimeout(() => {
                    let origin = window.location.origin + '/GrantmakingPortal/apex/IFPOCformPdf?id=' + this.parent.recordId + '&context=' + this.parent.context;
                    window.open(origin);
                }, 3000)
            ...
            __errorMessage(this.parent.context + ' Submitted!', this.parent.context + ' Submission Failed!', this.parent);
